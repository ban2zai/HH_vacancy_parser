...
def connect_to_mongodb():
    """
    Подключается к локальному экземпляру MongoDB и возвращает коллекцию.
    """
    try:
...
def save_vacancy_to_mongodb(collection, vacancy_data):
    ...
    # Инициализируем переменные для publication_date и publication_time
    publication_date_to_save = vacancy_data.get('Дата пуб')
    publication_time_to_save = vacancy_data.get('Время п')
    ...
    if existing_vacancy and "response_history" in existing_vacancy and existing_vacancy["response_history"]:
        ...
        try:
            ...
            # Проверка на наличие дублирующей записи по временному интервалу (менее 60 секунд)
            if (now_moscow - last_parsing_datetime) < timedelta(seconds=60):
                print(f"Обнаружена дублирующая запись для вакансии {vacancy_data['vacancy_id']} (временной интервал менее 60 секунд). Новая запись не будет добавлена.")
                return
        ...
        # Проверяем, изменились ли publication_date или publication_time
        last_publication_date = last_entry.get("publication_date")
        last_publication_time = last_entry.get("publication_time")
        
        if last_publication_date == publication_date_to_save and last_publication_time == publication_time_to_save:
            publication_date_to_save = None
            publication_time_to_save = None
    ...
    response_entry = {
        ...
        "publication_date": publication_date_to_save,
        "publication_time": publication_time_to_save,
...
def format_date_time_separate(iso_date_string):
    """
    Форматирует строку ISO 8601 в отдельные дату и время.
    """
...
def fetch_specializations_from_api(api_url: str) -> dict:
    """
    Получает справочник специализаций с API HH.ru.
    """
...
def create_specialization_lookup_table(json_data: dict) -> dict:
    """
    Создает словарь для быстрого поиска названий специализаций по их ID.
    """
    lookup_table = {}
...
def parse_hh_vacancies(base_url: str, max_pages: int = 0, specialization_lookup: dict = None) -> list[dict]:
    """
    Парсит вакансии с HH.ru по заданному URL.
    :param base_url: Базовый URL для поиска вакансий.
    :param max_pages: Максимальное количество страниц для парсинга (0 - все страницы).
    :param specialization_lookup: Словарь для преобразования ID специализаций в названия.
...
if __name__ == "__main__":
    ...
    if vacancies_collection is not None:
        base_url_to_parse = "https://krasnoyarsk.hh.ru/search/vacancy?hhtmFrom=main&hhtmFromLabel=vacancy_search_line&enable_snippets=false&area=54&professional_role=156&professional_role=160&professional_role=10&professional_role=12&professional_role=150&professional_role=25&professional_role=165&professional_role=34&professional_role=36&professional_role=73&professional_role=155&professional_role=96&professional_role=164&professional_role=104&professional_role=157&professional_role=107&professional_role=112&professional_role=113&professional_role=148&professional_role=114&professional_role=116&professional_role=121&professional_role=124&professional_role=125&professional_role=126&professional_role=70&professional_role=71&search_field=name&search_field=company_name&search_field=description&text="
        num_pages_to_parse = 20

        print("\nНачинаем парсинг вакансий...")
        all_extracted_data = parse_hh_vacancies(base_url_to_parse, num_pages_to_parse, specializations_map)
